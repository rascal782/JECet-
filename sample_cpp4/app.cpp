...
/* sample_c1マクロ */
#define GYRO_OFFSET  0          /* ジャイロセンサーオフセット値(角速度0[deg/sec]時) */
#define LIGHT_WHITE  55         /* 白色の光センサ値 */
#define LIGHT_BLACK  0          /* 黒色の光センサ値 */
/* sample_c2マクロ */
#define SONAR_ALERT_DISTANCE 30 /* 超音波センサーによる障害物検知距離[cm] */
/* sample_c3マクロ */
  ...
  while(1)
    {
      ...
      pwm_R = balancer.getPwmRight();       // <3>
      pwm_L = balancer.getPwmLeft();        // <3>
...
static int sonar_alert(void)
{
  ...
  if (++counter == 40/4) /* 約40msec周期毎に障害物検知  */
    {
      /*
       * 超音波センサーによる距離測定周期は、超音波の減衰特性に依存します。
